...
## Session log — 2026-07-10 (исправление: у Кацовича право ЕСТЬ, v5.14)

Пользователь усомнился в выводе «по ЗУ 76 право не зарегистрировано» («там же 3 страницы обычно так когда дают право»). Он прав.

По ЗУ 76 в Downloads лежат **два почти одноимённых файла**: `Кацович ЗУ Утлюкская76.pdf` (1 стр.) и
`Кацович ЗУ Утлюкская 76.pdf` (3 стр., с пробелом). Я читал короткий.
В трёхстраничной есть раздел 2: **право зарегистрировано 07.07.2026 15:51, общая долевая 1/2,
№ 94:21:1010002:4-26/108/2026-1**. Второй сособственник в выписке не назван (как и у Моршневой по Алмазной 43).

**Правило чтения выписок** (проверено по всем 16 одностраничным сканам):
- `Всего разделов: 1` → выписка только о характеристиках, раздела о правах не существует. Так у 94:21:1010002:1/2/3 — по ним приостановка, права нет.
- `Всего разделов: 2`, а страниц в PDF — 1 → **лист 2 просто не отсканирован**. Так у зданий 46 А/Б/В, 48 А/Б, 51 А/Б, 43 А и участков 12, 14, 47.

Формулировка в `registrations.json → pending` исправлена: было «выписка без раздела о правах»
(звучит как «права нет»), стало «в скане только лист 1 из 2 — статус права неизвестен».

⚠ Правка любого data-json требует бампа `BUILD_VERSION`, иначе `?v=` не обновится и браузер отдаст старый файл.

---

## Session log — 2026-07-10 (уведомления о приостановке, v5.5)

8 сканов CamScanner = **4 уникальных уведомления** Росреестра ХО о приостановлении (остальное — дубли:
12/1 → файлы 19.37+19.55, 15 → 19.38+19.53, 16 → 19.40+19.42+19.50, 12/2 → 19.52).
Двухстраничные сканы неполные — в них нет средней страницы (там срок и причина); полная версия трёхстраничная.

**Здесь впервые появились номера КУВД** (в выписках ЕГРН их нет — см. лог 2026-07-09).
Все 4 на Моршнева С.А., ул. Набережная, приостановка с 07.07.2026 **до 07.10.2026** (п. 5 ч. 1 ст. 26 ФЗ-218, 3 мес.):

| ЗУ | укр. участок | КУВД |
|----|--------------|------|
| 12/1 | 12:001:0246 | КУВД-101/2026-1773721/1 |
| 12/2 | 12:001:0245 | КУВД-101/2026-1773623/1 |
| 15   | 12:001:0244 | КУВД-101/2026-1774065/1 |
| 16   | 12:001:0277 | КУВД-101/2026-1774023/1 |

Причина одна на всех: Росреестр не может проверить достоверность украинского правоустанавливающего документа,
нужно решение региональной комиссии ХО (ФКЗ № 8-ФКЗ, Закон № 193-ФЗ). Минимущества ХО: +7 (860) 349-10-37, mizo@khogov.ru.

⚠️ Не путать: уведомления про **улицу Набережную**, а в базе есть ещё «Алмазная, ЗУ 15» (0211) и «Алмазная, ЗУ 16» (0207).
0246/0245/0244 имели `rr` (94:21-кадастры) → стали 🟡, зелёных осталось 15. Флажки: 80 🔴 / 8 🟢 / 4 🟡.

---

## Session log — 2026-07-10 (кнопка «ИНФО», v5.3)

Кнопка «📄 Регистрации» заменена на «📋 ИНФО» — реестр всех **132** объектов с адресом ФИАС ХО
(92 участка + 37 домов из `blds` + 3 `unbound`), сгруппирован по улицам, дома вложены под своим участком.

Цветовой статус: 🟢 `rr` (18 объектов) · 🟡 `dst:"stop"` · 🔴 `dst:"deny"` · ⚪ без документа (114).
Причина у 🟡/🔴 — из `dwhy`; у ⚪ подставляется `pending[].note` из registrations.json, иначе номер постановления ФИАС.
Номера регистрации права подтягиваются из registrations.json по кадастру РФ.

⚠️ `st` у участка уже занято исходником ДЗК («р») → статус документа хранится в `dst`/`dwhy`.
⚠️ `\b` в JS не работает с кириллицей: `/^(дом|здание)\b/` не матчит «дом 12» → нужен `\s`.
Поиск в модалке — по словам через «И» (иначе «набережная 49» не находит «улица Набережная, земельный участок 49»).
