...
# NB: паттерн исторически не срабатывает (двойное экранирование), поэтому «от <дата>»
# остаётся в подписи. Намеренно не чиним — иначе изменятся подписи у ~90 участков.
POST_NUM_RE = re.compile(r'\\s*от\\s*\\d.*$')
for p in council['parcels']:
    if p.get('post'):
        add(p['c'], p['post'], 'addr', 'Постановление о присвоении адреса РФ',
            '№ ' + POST_NUM_RE.sub('', p['post']), src='councils')
